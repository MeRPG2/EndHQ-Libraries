...
public class RemoteEntities
{
	...
	public static String COMPATIBLE_VERSION = "1.7.10";
	public static String COMPATIBLE_REVISION = "v1_7_R4";
	private static final String VERSION_FILE = "http://repo.infinityblade.de/re_versions.txt";
	public static String MINECRAFT_REVISION;
	...
	public void onEnable()
	{
		...
		VersionCheck:
		{
			...
			{
				inst.getLogger().severe("Invalid minecraft version for remote entities (Present: " + minecraftversion + ").");
				if(this.isAutoUpdateEnabled())
				{
					if(this.isVersionOnline(MINECRAFT_REVISION))
					{
						inst.getLogger().info("Version found online for MC " + minecraftversion + ". Initializing updating process.");
						this.updateTo(MINECRAFT_REVISION, minecraftversion);
						break VersionCheck;
					}
					else
						inst.getLogger().info("No new version found online.");
				}

				inst.getLogger().severe("Disabling plugin to prevent issues.");
			...
			else
			{
				try
				{
					ClassPath cp = this.m_pool.insertClassPath(new File(inst.getDataFolder(), "sources/").getAbsolutePath());
		...
		inst.getServer().getPluginManager().registerEvents(new DisableListener(), inst);
	}
	...
	private void checkClasses()
	{
		...
		File sources = new File(inst.getDataFolder(), "sources/");
		if(!sources.exists())
		{
			sources.mkdirs();
			try
			{
				BufferedReader reader = new BufferedReader(new InputStreamReader(inst.getResource("re_classes.txt")));
				String line;
	...
	void checkConfig()
	{
		inst.getConfig().set("autoUpdateSources", inst.getConfig().get("autoUpdateSources", false));
		COMPATIBLE_REVISION = inst.getConfig().getString("COMPATIBLE_REVISION", "v1_7_R4");
		COMPATIBLE_VERSION = inst.getConfig().getString("COMPATIBLE_VERSION", "1.7.10");
		inst.saveConfig();
	}

	public boolean isAutoUpdateEnabled()
	{
		return inst.getConfig().getBoolean("autoUpdateSources");
	}

	public void setAutoUpdateEnabled(boolean inEnabled)
	{
		inst.getConfig().set("autoUpdateSources", inEnabled);
		inst.getLogger().info("RemoteEntities auto updating has been enabled.");
	}
		...
		try
		{
			...
			for(String className : classes)
			{
				if(className != null && className.length() > 0)
				{
					...
					ctclass.writeFile(new File(inst.getDataFolder(), "sources").getAbsolutePath());
				}
			}

			inst.getConfig().set("COMPATIBLE_REVISION", inRevision);
			inst.getConfig().set("COMPATIBLE_VERSION", inVersion);
			inst.saveConfig();

			inst.getLogger().info("Loaded new classes. Please restart the server to apply changes.");
		}
		catch(Exception e)
		{
			e.printStackTrace();
